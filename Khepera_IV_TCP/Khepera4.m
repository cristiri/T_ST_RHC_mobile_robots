classdef Khepera4
    ...
    properties ( SetAccess = protected)
        client,host_address,port, pose, wheels_vel, desired_max_speed, connected, enc_pos_left, enc_pos_right, control_parameters
    end
    properties (Constant)
    ...
    methods
        ...
        function obj = Khepera4(host_address,port,pose,control_parameters,desired_max_speed)
            obj.connected=false;
            obj.wheels_vel=[0;0];
            obj.enc_pos_left=0;
            obj.enc_pos_right=0;
        ...
        function [obj,result]=connect(obj)
            if obj.connected~=true
                obj.client=tcpclient(obj.host_address,obj.port);
        ...
        function set_motors_speed(obj,w_l,w_r)
            if obj.connected==false
                error("NOT connected to the server. Call connect().")
            end
            cmd=['$SET_SPEED,' num2str(w_l) ',' num2str(w_r)];
        ...
        function reset_encoders(obj)
            %Reset the encoder to [0 0] by sending the proper command
            if obj.connected==false
                error("NOT connected to the server. Call connect().")
            end
            writeline(obj.client,'$RESET_ENC');
            readline(obj.client);
            flush(obj.client);
        end

        function [left_speed,right_speed]=read_wheels_speed(obj)
            if obj.connected==false
                error("NOT connected to the server. Call connect().")
            end
            %Sending the wheel_speed reading command
            writeline(obj.client,'$GET_SPEED');
            resp=readline(obj.client);
            flush(obj.client);
            tkns=split(resp,',');
            left_speed=str2double(tkns(2));
            right_speed=str2double(tkns(3));
        end

        function ultrasound_off(obj)
            %Send the ultrasound off command
            if obj.connected==false
                error("NOT connected to the server. Call connect().")
            end
            writeline(obj.client,'$DISABLE_US');
            readline(obj.client);
            flush(obj.client);
        end

        function status=get_battery_status(obj)
            %Get the battery absolute capacity
            if obj.connected==false
                error("NOT connected to the server. Call connect().")
            end
            writeline(obj.client,'$GET_BATTERY_CAPACITY');
            status=readline(obj.client);
            flush(obj.client);
        end
        ...
        function obj=ST_MPC_trajectory_tracking(obj)

            function uk= online_phase(x_curr,set_to_reach,A,B,inputconstraint,R,R_d,u_pre)
                %move between polytope
                ...
                % tic
                [uk,~,~,~,~] = quadprog(H,f,A_constr,b_constr,Aeq,beq,lb,ub,x0,options);
            ...
            z_curr=[x+obj.control_parameters.b*cos(theta)-(obj.x_r+obj.control_parameters.b*cos(obj.theta_r)) ;
                y+obj.control_parameters.b*sin(theta)-(obj.y_r+obj.control_parameters.b*sin(obj.theta_r)) ];

            T_FL  = [cos(theta) sin(theta); -sin(theta)/obj.control_parameters.b cos(theta)/obj.control_parameters.b];


            %%%%%%

            Hd=[-1/obj.desired_max_speed_rad 0 ;
                0 -1/obj.desired_max_speed_rad ;
                1/obj.desired_max_speed_rad 0 ;
                0 1/obj.desired_max_speed_rad];  %shaping matrix of box-like constraint set for differential-drive


            H_curr=Hd/obj.M*T_FL;
            g=ones(4,1);
            U_curr=Polyhedron(H_curr,g);



            for j=2:size(obj.control_parameters.Pn,1)
                if (obj.control_parameters.Pn(j,1).contains(z_curr))
                    index=j;
                    break
                end
            end
            ...
            [uk] = online_phase(z_curr,obj.control_parameters.Pn(index-1,1),obj.control_parameters.Ad,obj.control_parameters.Bd,U_curr,obj.control_parameters.R,obj.control_parameters.Rd,obj.control_parameters.u_pre);
            obj.control_parameters.u_pre=uk;
        ...
        function obj=ST_MPC_disturbance_trajectory_tracking(obj)
            function uk= online_phase(x_curr,Qpre,A,B,Hu,gu,R,Rd,ur,u_pre)



                %Set theoretic MPC online optimization
                obj_fun=@(u)(A*x_curr+B*(u))'*(A*x_curr+B*(u))+u'*R*u+(u-u_pre)'*Rd*(u-u_pre);


                function [c,ceq]=quad_constr(u,Hu,gu,Qpre,ur,A,B,x_curr)
                    c1=Hu*(u+ur)-gu;
                    c2=(A*x_curr+B*(u))'*inv(Qpre)*(A*x_curr+B*(u))-1;
                    c=[c1;c2];
                    ceq=[];
                end
                ...
                usol = fmincon(obj_fun,[0;0],[],[],[],[],[],[],@(u)quad_constr(u,Hu,gu,Qpre,ur,A,B,x_curr),options);
                uk=usol;
            ...
            T_FL_r=[cos(obj.theta_r) sin(obj.theta_r); -sin(obj.theta_r)/obj.control_parameters.b cos(obj.theta_r)/obj.control_parameters.b];
            ur=inv(T_FL_r)*[obj.v_r;obj.w_r];



            isintern=0;
            for j=1:2:length(obj.control_parameters.Q_k)
                Q_curr=obj.control_parameters.Q_k(1:2,j:j+1);
                if  z_curr'*inv(Q_curr)*z_curr<=1

                    index=(j-1)/2+1;
                    obj.control_parameters.index_seq=[obj.control_parameters.index_seq index];
                    isintern=1;
                    if index>1
                        Qpre=obj.control_parameters.Q_k(1:2,j-2:j-1);
                    end
                    break
                end
            end
            ...
            if index~=1
                ukhat = online_phase(z_curr,Qpre,obj.control_parameters.Ad,obj.control_parameters.Bd,H_curr,g,obj.control_parameters.R,obj.control_parameters.Rd,ur,obj.control_parameters.u_pre);
            else
                ukhat=-inv(obj.control_parameters.Bd)*z_curr;
            end
            uk=ukhat+ur;


            obj.control_parameters.u_pre=ukhat;
